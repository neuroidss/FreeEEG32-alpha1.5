...
// #define TERM_SPEED CBR_921600 // FreeEEG32a1_5 is a bit faster than others
...
//#define TERM_SPEED 462427 // FreeEEG32a1_5 is a bit faster than others
// #define TERM_SPEED 2000000 // FreeEEG32a1_5 is a bit faster than others
...
// #define TERM_SPEED 1000000
// #define TERM_SPEED 1152000
// #define TERM_SPEED 1843200
...
{
	return "FreeEEG32a1_5";
}

//___________________________________________________________________//
//                                                                   //

void  CDriverFreeEEG32a1_5::updateDaisy(bool bBeQuiet) {
...
//noacc	m_vSampleAccBuffer.resize(AccNbValuesPerSample);
//noacc	m_vSampleAccBufferDaisy.resize(AccNbValuesPerSample);
	
	// init buffer for 1 EEG value and 1 accel value
	m_vEEGValueBuffer.resize(EEGValueBufferSize);
//noacc	m_vAccValueBuffer.resize(AccValueBufferSize);
...
int32 CDriverFreeEEG32a1_5::interpret24bitAsInt32(std::vector < uint8 > byteBuffer) {
...
//dma
int32 CDriverFreeEEG32a1_5::interpret24bitAsInt32_dma(std::vector < uint8 > byteBuffer) {
	// create a big endian so that we could adapt to host architecture later on
	int32 newInt = (byteBuffer[0] << 24) | (byteBuffer[1] << 16) | byteBuffer[2] << 8;
	// depending on most significant byte, set positive or negative value
	if ((newInt & 0x00008000) > 0) {
		newInt |= 0x000000FF;
	} else {
...
int32 CDriverFreeEEG32a1_5::interpret16bitAsInt32(std::vector < uint8 > byteBuffer) {
	// create a big endian so that we could adapt to host architecture later on
...
//					if (m_ui16ExtractPosition % 8 == 0)
//					{
//						m_vSampleEEGBuffer[m_ui16ExtractPosition] = (float) interpret24bitAsInt32(m_vEEGValueBuffer)*(ScaleFacuVoltsPerCount*AD7770_GAIN);
//					}
//					else
//					{
//dma
					if(EEGValueBufferSize==4)
					{
						m_vSampleEEGBuffer[m_ui16ExtractPosition] = (float) interpret24bitAsInt32_dma(m_vEEGValueBuffer)*ScaleFacuVoltsPerCount;
					} else {
...
boolean CDriverFreeEEG32a1_5::initTTY(::FD_TYPE* pFileDescriptor, uint32 ui32TTYNumber)
{
...
boolean CDriverFreeEEG32a1_5::boardWriteAndPrint(::FD_TYPE i32FileDescriptor, const char * cmd, bool waitForResponse, uint32 sleepBetween) {
...
boolean CDriverFreeEEG32a1_5::initBoard(::FD_TYPE i32FileDescriptor)
{
...
void CDriverFreeEEG32a1_5::fastReco(::FD_TYPE i32FileDescriptor)
{
...
void CDriverFreeEEG32a1_5::closeTTY(::FD_TYPE i32FileDescriptor)
{
#if defined TARGET_OS_Windows
...
bool CDriverFreeEEG32a1_5::handleCurrentSample(int32 packetNumber) {
...
//noacc			l_vSampleBuffer.reserve(m_vSampleEEGBuffer.size() + m_vSampleAccBuffer.size());
			l_vSampleBuffer.reserve(m_vSampleEEGBuffer.size());
			l_vSampleBuffer.insert(l_vSampleBuffer.end(), m_vSampleEEGBuffer.begin(), m_vSampleEEGBuffer.end());
//noacc			l_vSampleBuffer.insert(l_vSampleBuffer.end(), m_vSampleAccBuffer.begin(), m_vSampleAccBuffer.end());
...
int32 CDriverFreeEEG32a1_5::readPacketFromTTY(::FD_TYPE i32FileDescriptor)
{
