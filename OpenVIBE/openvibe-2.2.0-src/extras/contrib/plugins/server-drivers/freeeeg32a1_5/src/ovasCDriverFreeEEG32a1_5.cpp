...
#include "ovasCDriverFreeEEG32a1_5.h"
...
#define Token_MissingSampleDelayBeforeReset       "AcquisitionDriver_FreeEEG32a1_5_MissingSampleDelayBeforeReset"
#define Token_DroppedSampleCountBeforeReset       "AcquisitionDriver_FreeEEG32a1_5_DroppedSampleCountBeforeReset"
#define Token_DroppedSampleSafetyDelayBeforeReset "AcquisitionDriver_FreeEEG32a1_5_DroppedSampleSafetyDelayBeforeReset"
...
CDriverFreeEEG32a1_5::CDriverFreeEEG32a1_5(IDriverContext& rDriverContext)
	:IDriver(rDriverContext)
	,m_oSettings("AcquisitionServer_Driver_FreeEEG32a1_5", m_rDriverContext.getConfigurationManager())
	,m_pCallback(NULL)
...
void CDriverFreeEEG32a1_5::release(void)
{
	delete this;
}

const char* CDriverFreeEEG32a1_5::getName(void)
{
...
void CDriverFreeEEG32a1_5::updateDaisy(bool bQuietLogging)
{
	// change channel and sampling rate according to daisy module
	CConfigurationFreeEEG32a1_5::SDaisyInformation l_oDaisyInformation=CConfigurationFreeEEG32a1_5::getDaisyInformation(m_bDaisyModule?CConfigurationFreeEEG32a1_5::DaisyStatus_Active:CConfigurationFreeEEG32a1_5::DaisyStatus_Inactive);
...
boolean CDriverFreeEEG32a1_5::initialize(
	...
	IDriverCallback& rCallback)
{
	...
	// init state
	m_ui32ReadState=CDriverFreeEEG32a1_5::ParserAutomaton_Default;
...
//	if(!this->resetBoard(m_i32FileDescriptor, true))
	...
	m_f32UnitsToRadians = (float32) (0.002 / pow(2.,4)); // @aj told me - this is undocumented and may have been taken from the FreeEEG32a1_5 plugin for processing
...
boolean CDriverFreeEEG32a1_5::start(void)
{
	if(!m_rDriverContext.isConnected())
	{
		return false;
...
boolean CDriverFreeEEG32a1_5::loop(void)
{
	if(!m_rDriverContext.isConnected())
	{
		return false;
...
boolean CDriverFreeEEG32a1_5::stop(void)
{
	if(!m_rDriverContext.isConnected())
	{
		return false;
...
boolean CDriverFreeEEG32a1_5::uninitialize(void)
{
	if(!m_rDriverContext.isConnected())
	{
		return false;
...
boolean CDriverFreeEEG32a1_5::isConfigurable(void)
{
	return true;
}

boolean CDriverFreeEEG32a1_5::configure(void)
{
	CConfigurationFreeEEG32a1_5 m_oConfiguration(
		OpenViBE::Directories::getDataDir() + "/applications/acquisition-server/interface-FreeEEG32a1_5.ui",
...
int32 CDriverFreeEEG32a1_5::interpret24bitAsInt32(const std::vector < uint8 >& byteBuffer)
{
...
int32 CDriverFreeEEG32a1_5::interpret16bitAsInt32(const std::vector < uint8 >& byteBuffer)
{
	// create a big endian so that we could adapt to host architecture later on
...
OpenViBE::int16 CDriverFreeEEG32a1_5::parseByte(uint8 ui8Actbyte)
{
	...
	switch(m_ui32ReadState)
	{
		...
		case CDriverFreeEEG32a1_5::ParserAutomaton_Default:
			...
			else
			{
				...
				{
					m_ui32ReadState = CDriverFreeEEG32a1_5::ParserAutomaton_StartByteReceived;
				}
		...
		case CDriverFreeEEG32a1_5::ParserAutomaton_StartByteReceived:
			m_i16SampleNumber = ui8Actbyte;
			m_ui32ReadState = CDriverFreeEEG32a1_5::ParserAutomaton_SampleNumberReceived;
			break;
		 ...
		 */
		case CDriverFreeEEG32a1_5::ParserAutomaton_SampleNumberReceived:
...
//					if (m_ui32ExtractPosition % 8 == 0)
//					{
//						m_vSampleEEGBuffer[m_ui32ExtractPosition] = (float) this->interpret24bitAsInt32(m_vEEGValueBuffer)*(m_f32UnitsToMicroVolts*AD7770_GAIN);
//					}
//					else
//					{
...
//					}
			...
			if(m_ui32ExtractPosition == EEGValueCountPerSample)
			{
				...
				m_ui32ReadState = CDriverFreeEEG32a1_5::ParserAutomaton_EEGSamplesReceived;
		 ...
		 */
		case CDriverFreeEEG32a1_5::ParserAutomaton_EEGSamplesReceived:
			...
			{
				// next step: footer
				m_ui32ReadState = CDriverFreeEEG32a1_5::ParserAutomaton_AccelerometerSamplesReceived;
		...
		case CDriverFreeEEG32a1_5::ParserAutomaton_AccelerometerSamplesReceived:
...
boolean CDriverFreeEEG32a1_5::sendCommand(::FD_TYPE i32FileDescriptor, const char* sCommand, bool bWaitForResponse, bool bLogResponse, uint32 ui32Timeout, std::string& sReply)
{
	...
	{
		m_rDriverContext.getLogManager() << LogLevel_Trace << this->m_sDriverName << ": Sending sequence to FreeEEG32a1_5 board [" << std::string(1, sCommand[i]).c_str() << "]\n";
...
boolean CDriverFreeEEG32a1_5::resetBoard(::FD_TYPE i32FileDescriptor, bool bRegularInitialization)
{
	...
	if(bRegularInitialization)
	{
		...
		while(std::getline(l_oCommandReplyInputStringStream, l_sLine, '\n'))
		{
			if(l_sLine != "$$$")
			{
				::sscanf(l_sLine.c_str(), "FreeEEG32a1_5 V%u %u channel", &m_oDeviceInformation.ui32DeviceVersion, &m_oDeviceInformation.ui32DeviceChannelCount);
		...
		// verifies FreeEEG32a1_5 board presence
		if(m_oDeviceInformation.ui32DeviceVersion==0x00)
		{
			m_rDriverContext.getLogManager() << LogLevel_Error << this->m_sDriverName << ": The attached device does not look like an FreeEEG32a1_5 device\n";
			return false;
		}
...
bool CDriverFreeEEG32a1_5::handleCurrentSample(int32 i32PacketNumber)
{
	...
	if(i32PacketNumber >= 0)
	{
		...
		if(!m_bDaisyModule)
		{
			...
			// YRD NOTE: FreeEEG32a1_5 ACCELEROMETERS ARE SAMPLED WAY BELOW EEG (~25Hz, depends upon firmware)
...
boolean CDriverFreeEEG32a1_5::openDevice(::FD_TYPE* pFileDescriptor, uint32 ui32TTYNumber)
{
	CString l_sTTYName;
	...
	if(ui32TTYNumber == UNDEFINED_DEVICE_IDENTIFIER)
	{
		...
		do
		{
			l_sTTYName = CConfigurationFreeEEG32a1_5::getTTYFileName(i++);
			if(CConfigurationFreeEEG32a1_5::isTTYFile(l_sTTYName))
			{
				m_rDriverContext.getLogManager() << LogLevel_Info << this->m_sDriverName << ": Automatically picked port [" << i << ":" << l_sTTYName << "]\n";
				l_bSuccess=true;
			}
		}
		while(!l_bSuccess && i<CConfigurationFreeEEG32a1_5::getMaximumTTYCount());
	...
	{
 		l_sTTYName = CConfigurationFreeEEG32a1_5::getTTYFileName(ui32TTYNumber);
	}
...
void CDriverFreeEEG32a1_5::closeDevice(::FD_TYPE i32FileDescriptor)
{
#if defined TARGET_OS_Windows
...
uint32 CDriverFreeEEG32a1_5::writeToDevice(::FD_TYPE i32FileDescriptor, const void* pBuffer, const uint32 ui32BufferSize)
{
...
uint32 CDriverFreeEEG32a1_5::readFromDevice(::FD_TYPE i32FileDescriptor, void* pBuffer, const uint32 ui32BufferSize, const uint64 ui64TimeOut)
{
#if defined TARGET_OS_Windows
